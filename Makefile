...
run: ## Starts the local server with production settings
	ESMERALD_SETTINGS_MODULE=esmerald_auth.configs.development.settings.DevelopmentAppSettings python -m esmerald_auth.serve

.PHONY: test
test: ## Runs the local tests with nose
	ESMERALD_SETTINGS_MODULE=esmerald_auth.configs.testing.settings.TestingAppSettings pytest -s $(TESTONLY)
